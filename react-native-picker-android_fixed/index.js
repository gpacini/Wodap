...
export class PickerAndroidItem extends Component{
...
export default class PickerAndroid extends Component{
	...
	constructor(props, context){
		...
		this.middleViewHeight = this.state.itemStyle.fontSize ? this.state.itemStyle.fontSize+14 : 40;
		middleViewHeight = this.middleViewHeight;
	}
	...
	_move(dy){
		...
		this.middleHeight = Math.abs(-index * this.middleViewHeight + dy);
		this.up && this.up.setNativeProps({
			style: {
				marginTop: (1.5 - index) * (this.middleViewHeight - 10) + dy * .75,
			},
		});
		this.middle && this.middle.setNativeProps({
			style: {
				marginTop: -index * this.middleViewHeight + dy,
			},
		});
		this.down && this.down.setNativeProps({
			style: {
				marginTop: (-index - 1) * (this.middleViewHeight - 10) + dy * .75,
			},
		});
	}
	...
	_moveTo(index){
		...
		if(diff && !this.isMoving) {
			marginValue = diff * this.middleViewHeight;
	...
	_handlePanResponderMove(evt, gestureState){
		...
		// turn down
		if(dy > 0) {
			this._move(dy > this.index * this.middleViewHeight ? this.index * this.middleViewHeight : dy);
		}else{
			this._move(dy < (this.index - this.state.items.length + 1) * this.middleViewHeight ? (this.index - this.state.items.length + 1) * this.middleViewHeight : dy);
		}
	}

	_handlePanResponderRelease(evt, gestureState){
		let middleHeight = this.middleHeight;
		this.index = middleHeight % this.middleViewHeight >= 20 ? Math.ceil(middleHeight / this.middleViewHeight) : Math.floor(middleHeight / this.middleViewHeight);
		this._move(0);
		this._onValueChange();
	}
	...
	_renderItems(items){
		...
		items.forEach((item, index) => {
			...
			upItems[index] = <Text
								key={'up'+index}
								style={[styles.upText, this.state.itemStyle, {height:this.middleViewHeight-10}]}
								onPress={() => {
									this._moveTo(index);
								}} >
								{item.label}
			...
			middleItems[index] = <Text
									key={'mid'+index}
									style={[styles.middleText, this.state.itemStyle, {height:this.middleViewHeight}]}>{item.label}
								</Text>;

			downItems[index] = <Text
									key={'down'+index}
									style={[styles.downText, this.state.itemStyle, {height:this.middleViewHeight-10}]}
									onPress={() => {
										this._moveTo(index);
									}} >
									{item.label}
	...
	render(){
		...
		let upViewStyle = {
			marginTop: (1.5 - index) * (this.middleViewHeight-10),
			height: length * (this.middleViewHeight-10),
		};
		let middleViewStyle = {
			marginTop:  -index * this.middleViewHeight,
		};
		let downViewStyle = {
			marginTop: (-index - 1) * (this.middleViewHeight-10),
			height:  length * (this.middleViewHeight-10),
		};
		...
		return (
			...
			<View style={[styles.container, this.state.pickerStyle]} {...this._panResponder.panHandlers}>
				...
				<View style={[styles.up, {zIndex:1}]}>
					<View style={[styles.upView, upViewStyle, {zIndex:1}]} ref={(up) => { this.up = up }} >
						{ items.upItems }
					</View>
				</View>

				<View style={[styles.middle, {height:this.middleViewHeight}]}>
					<View style={[styles.middleView, middleViewStyle, {height:this.middleViewHeight}]} ref={(middle) => { this.middle = middle }} >
						{ items.middleItems }
					</View>
				</View>

				<View style={[styles.down, {zIndex:1}]}>
					<View style={[styles.downView, downViewStyle, {zIndex:1}]} ref={(down) => { this.down = down }} >
						{ items.downItems }
					</View>
				</View>
			</View>
